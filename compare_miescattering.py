...
ssd_mie, smd_mie, P_coeffs = gmd.calc_arts_scattering_data(
    f_grid, t_grid, za_grid, droplet_radius, [1], m, rho_water
)
...
for i in range(len(f_grid)):
    sigma_sca_arts_from_int[i] = (
        np.trapz(
            ssd_arts.pha_mat_data[i, -1, :, 0, 0, 0, 0], -np.cos(za_grid * np.pi / 180)
        ) 
        * 2
        * np.pi
    )
    sigma_sca_mie_from_int[i] = (
        np.trapz(
            ssd_mie.pha_mat_data[i, 0, :, 0, 0, 0, 0], -np.cos(za_grid * np.pi / 180)
        )
        * 2
        * np.pi
    )
...
sigma_sca_arts = (
    ssd_arts.ext_mat_data[:, -1, 0, 0, 0] - ssd_arts.abs_vec_data[:, -1, 0, 0, 0]
)
sigma_sca_mie = (
    ssd_mie.ext_mat_data[:, 0, 0, 0, 0] - ssd_mie.abs_vec_data[:, 0, 0, 0, 0]
)
...
ax[0].loglog(
    f_grid * 1e-9, ssd_mie.ext_mat_data[:, 0, 0, 0, 0], "+-", label="Miepython SSD"
)
ax[0].loglog(
    f_grid * 1e-9, ssd_arts.ext_mat_data[:, -1, 0, 0, 0], "x--", label="ARTS SSD"
)
...
ax[2].loglog(
    f_grid * 1e-9, ssd_mie.abs_vec_data[:, 0, 0, 0, 0], "+-", label="Miepython SSD"
)
ax[2].loglog(
    f_grid * 1e-9, ssd_arts.abs_vec_data[:, -1, 0, 0, 0], "x--", label="ARTS SSD"
)
...
for i, fidx_i in enumerate(freq_idx):
    for j, coeff in enumerate(P_coeffs):
        ...
        if j == 0:
            ...
            X_mie = (
                ssd_mie.pha_mat_data[fidx_i, 0, :, 0, 0, 0, j]
                / ssd_mie.pha_mat_data[fidx_i, 0, :, 0, 0, 0, 0]
            )
            X_arts = (
                ssd_arts.pha_mat_data[fidx_i, -1, :, 0, 0, 0, j]
                / ssd_arts.pha_mat_data[fidx_i, -1, :, 0, 0, 0, 0]
            )
...
phfct_integral_mie, phfct_integral_mie_test = gmd.integrate_phasefunction_for_testing(
    ssd_mie
)
phfct_integral_arts, phfct_integral_arts_test = gmd.integrate_phasefunction_for_testing(
    ssd_arts, t_index=-1
)
...
ax.semilogx(
    f_grid * 1e-9, (phfct_integral_mie / 2 - 1) * 100, "-", label="Miepython SSD"
)
ax.semilogx(f_grid * 1e-9, (phfct_integral_arts / 2 - 1) * 100, "--", label="ARTS SSD")
ax.semilogx(
    f_grid * 1e-9,
    (phfct_integral_mie_test / 2 - 1) * 100,
    "o",
    label="Miepython SSD test",
)
ax.semilogx(
    f_grid * 1e-9, (phfct_integral_arts_test / 2 - 1) * 100, "s", label="ARTS SSD test"
)
