...
def S1S2ToPhaseMatSphere(S11, S22):
    ...
    P[:, 0] = (np.abs(S11) ** 2 + np.abs(S22) ** 2) / 2.0
    P[:, 1] = (np.abs(S11) ** 2 - np.abs(S22) ** 2) / 2.0
    P[:, 2] = P[:, 0]
...
def calc_mie_scattering(
    ...
    if verbose:
        test = (
            np.trapz(
                (np.abs(S1_hr) ** 2 + np.abs(S2_hr) ** 2) * np.sin(za_rad_hr),
                x=za_rad_hr,
            )
            * np.pi
            * qext
...
def calc_arts_scattering_data(
    ...
    description = (
        "Spherical particle of liquid water generated using\n"
        + f"miepython and python by Manfred Brath, {date.today()}\n"
    )

    source = (
        f"miepython {miepython.__version__}: https://github.com/scottprahl/miepython/"
    )
    ...
    for k, f_k in enumerate(f_grid[logic_x]):
        ...
        if x[k] < 10000 or ignore_limit:
            ...
            for j in range(N_sub):
                ...
                P_j, sigma_ext_j, sigma_abs_j, P_coeffs = calc_mie_scattering(
                    r_ij,
                    f_k,
                    za_grid,
                    m[k],
            ...
            Csca = (
                np.trapz(
                    np.squeeze(pha_mat[k, 0, :, 0, 0, 0, 0]) * np.sin(za_rad), za_rad
                )
                * 2
                * np.pi
            )
    ...
    size_description = (
        f"Geometric mean of the scattering properties of {N_sub} \n"
    ...
    ssd.description = (
        description
        + f"mean particle size:{droplet_radius*1e6:.5f}µm\n"
        + size_description
    )
...
def integrate_phasefunction_for_testing(ssd, t_index=0):
    ...
    for i, f_i in enumerate(ssd.f_grid):
        ...
        sca = (
            ssd.ext_mat_data[i, t_index, 0, 0, 0]
            - ssd.abs_vec_data[i, t_index, 0, 0, 0]
        )
        ...
        if sca > 0:
            ...
            kernel = (
                (phfct[0:-1] * np.sin(za_rad[0:-1]) + phfct[1:] * np.sin(za_rad[1:]))
                / 2
                * (za_rad[1:] - za_rad[0:-1])
            )
